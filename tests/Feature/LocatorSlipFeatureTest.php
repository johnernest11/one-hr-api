...
class LocatorSlipFeatureTest extends TestCase
{
    ...
    public function test_it_can_view_grouped_locator_slip(): void
    {
        ...
        $response->assertStatus(200);
        $response->assertJsonCount(3, 'data'); // this will be the two employees + 1 created by default
    }
    ...
    public function test_it_can_search_my_locator_by_ls_no(): void
    {
        ...
        $response = $this->withToken($this->authTokenAdmin)->getJson($this->uriWithId.'/'.$employee->id.'/'.'locator-slips/search?query='.$lsNo);
        $response->assertStatus(200);

        $this->assertEquals($lsNo, $response['data'][0]['locator_slip_no']);

    }

    public function test_it_can_search_all_locator_by_employee_name(): void
    {
        $individual_1 = IndividualBasicDetail::factory()->create();
        $employee_1 = Employee::whereBelongsTo($individual_1)->firstOrFail();
        ...
        $employeeName = $individual_1->first_name;

        $response = $this->withToken($this->authTokenAdmin)->getJson($this->baseUri.'/search-all?query='.$employeeName);
        $response->assertStatus(200);
        $response->assertJsonCount(1, 'data'); // there should only be one employee

        $this->assertEquals($employeeName, $response['data'][0]['individual_basic_detail']['first_name']);

    }

    public function test_it_can_filter_my_locators_by_date(): void
    {
        $myProf = $this->user_admin->userProfile;
        $ownData = IndividualBasicDetail::factory()->withExistingUserProfile($myProf)->create();
        $employee = Employee::whereBelongsTo($ownData)->firstOrFail();
        ...
        $date = now()->toDateString();
        $locatorSlip = LocatorSlip::factory()->create(['employee_id' => $employee->id, 'date' => $date]);
        $extraLocators = LocatorSlip::factory(5)->setEmployee($employee->id)->create();

        $response = $this->withToken($this->authTokenAdmin)->getJson($this->uriWithId.'/'.$employee->id.'/locator-slips?date-filter='.$date);
        $response->assertStatus(200);
        $this->assertEquals($locatorSlip->id, $response['data'][0]['id']);

    }

    public function test_it_can_filter_my_locators_by_form_type(): void
    {
        $myProf = $this->user_admin->userProfile;
        $ownData = IndividualBasicDetail::factory()->withExistingUserProfile($myProf)->create();
        $employee = Employee::whereBelongsTo($ownData)->firstOrFail();

        $formType = LocatorFormType::FORM_C->value;
        $locatorSlip = LocatorSlip::factory()->create(['employee_id' => $employee->id, 'form_type' => $formType]);
        $extraLocators = LocatorSlip::factory(5)->setEmployee($employee->id)->create();

        $response = $this->withToken($this->authTokenAdmin)->getJson($this->uriWithId.'/'.$employee->id.'/locator-slips?form-type='.$formType);
        $response->assertStatus(200);
    ...
    public function test_it_can_filter_all_locators_by_date(): void
    {
        $individual_1 = IndividualBasicDetail::factory()->create();
        $employee_1 = Employee::whereBelongsTo($individual_1)->firstOrFail();
        ...
        $date = now()->toDateString();
        $locatorSlip = LocatorSlip::factory()->create(['employee_id' => $employee_1->id, 'date' => $date]);
        $locatorSlips_2 = LocatorSlip::factory(5)->setEmployee($employee_2)->create();

        $response = $this->withToken($this->authTokenAdmin)->getJson($this->baseUri.'/'.'grouped?date-filter='.$date);
        $response->assertStatus(200);
        $response->assertJsonFragment([
            'id' => $locatorSlip->id,
        ]);
    ...
    public function test_it_can_filter_all_locators_by_form_type(): void
    {
        $individual_1 = IndividualBasicDetail::factory()->create();
        $employee_1 = Employee::whereBelongsTo($individual_1)->firstOrFail();
        ...
        $formType = LocatorFormType::FORM_C->value;
        $locatorSlip = LocatorSlip::factory()->create(['employee_id' => $employee_1->id, 'form_type' => $formType]);
        $locatorSlips_2 = LocatorSlip::factory(5)->setEmployee($employee_2)->create();

        $response = $this->withToken($this->authTokenAdmin)->getJson($this->baseUri.'/'.'grouped?form-type='.$formType);
        $response->assertStatus(200);
        $response->assertJsonFragment([
            'id' => $locatorSlip->id,
        ]);
    ...
    public function test_it_can_filter_all_locators_by_office(): void
    {
        $individual_1 = IndividualBasicDetail::factory()->create();
        $employee_1 = Employee::whereBelongsTo($individual_1)->firstOrFail();

        $office_id = Office::inRandomOrder()->first()->id;
        $employee_1->update(['office_id' => $office_id]);
        ...
        $response = $this->withToken($this->authTokenAdmin)->getJson($this->baseUri.'/'.'grouped?office='.$office_id);
        $response->assertStatus(200);
        $response->assertJsonFragment([
    ...
    public function test_it_can_filter_all_locators_by_division(): void
    {
        $individual_1 = IndividualBasicDetail::factory()->create();
        $employee_1 = Employee::whereBelongsTo($individual_1)->firstOrFail();
        ...
        $response = $this->withToken($this->authTokenAdmin)->getJson($this->baseUri.'/'.'grouped?division='.$division_id);
        $response->assertStatus(200);
        $response->assertJsonFragment([
            'employee_id' => $employee_1->id,
        ]);
    ...
    public function test_it_can_filter_all_locators_by_section(): void
    {
        $individual_1 = IndividualBasicDetail::factory()->create();
        $employee_1 = Employee::whereBelongsTo($individual_1)->firstOrFail();

        $sec_id = SectionOrUnit::inRandomOrder()->first()->id;
        $employee_1->update(['section_or_unit_id' => $sec_id]);
        ...
        $response = $this->withToken($this->authTokenAdmin)->getJson($this->baseUri.'/'.'grouped?section='.$sec_id);
        $response->assertStatus(200);
        $response->assertJsonFragment([
            'employee_id' => $employee_1->id,
        ]);
